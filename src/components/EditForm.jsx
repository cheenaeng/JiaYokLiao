...
function EditForm({ medicationEdit, medData, updateEditStatus }) {
    ...
    axios.get(`/medicationEdit/${medicationEdit[0].id}`)
      .then((response) => {
        ...
        const initialRecordedFormValues = {
          id: response.data.allMedRecords.id,
    ...
    return (
      <HStack>
        <NumberInput name="medDose" value={formInput.medDose} onChange={(valueString) => setFormInput({ ...formInput, medDose: valueString })} min={0}>
          <NumberInputField type="number" className="form-input-field" />
          <NumberInputStepper>
  ...
  function SaveEdits() {
    const saveFormEdit = () => {
      ...
      axios.put('/formData', allData)
        .then((response) => {
          // clearAllData();
          console.log(response.data);
          updateEditStatus((prev) => prev = true);
          setNewRecord((prevRecord) => [...prevRecord, response.data.editedRecord]);
    ...
    return (
      <Flex justify="center">
        <Button className="user-button" mt={3} onClick={() => { saveFormEdit(); }}>
          Save Record
        </Button>
      </Flex>
    );
